...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_hOwner[0x4]; // 0xa50            
            bool m_bHadOwner; // 0xa54            
            uint8_t _pad0a55[0x3]; // 0xa55
            float m_flPostSpeakDelay; // 0xa58            
            float m_flPreDelay; // 0xa5c            
            bool m_bIsBackground; // 0xa60            
            bool m_bRemoveOnCompletion; // 0xa61            
            uint8_t _pad0a62[0x2]; // 0xa62
            // m_hTarget has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::server::CBaseEntity> m_hTarget;
            char m_hTarget[0x4]; // 0xa64            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CInstancedSceneEntity because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CInstancedSceneEntity) == 0xa68);
    };
};
